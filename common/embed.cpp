...
{
    nsresult rv =
      mPrivate->webNavigation->LoadURI(NS_ConvertUTF8toUTF16(uri).get(),
...
{
    nsresult rv =
      mPrivate->webNavigation->Stop(nsIWebNavigation::STOP_ALL);
    return rv;
}
...
{
    nsresult rv =
      mPrivate->webNavigation->Reload(nsIWebNavigation::LOAD_FLAGS_NONE);
    return rv;
}
...
{
    nsresult rv =
      mPrivate->webNavigation->GoForward();
    return rv;
}
